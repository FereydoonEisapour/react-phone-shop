...
function Cart({ img, title, price, count, id }) {
  const productContext = useContext(ProductContext);
  let removeCart = () => {
    productContext.dispatch({ type: "remove_cart", payload: { id } });
  };
  let idHandler = () => {
    productContext.dispatch({ type: "id_handler", payload: { id } });
  };
  let increment = () => {
    productContext.dispatch({ type: "increment", payload: { id } });
  };
  let decrement = () => {
    productContext.dispatch({ type: "decrement", payload: { id } });
  };
  return (
      ...
      <div className="row col-12 d-flex justify-content-between shadow pt-3 mb-3" id="now">
        ...
        <div className="col-9 d-flex row ">
          ...
          <div className="mt-3 mb-2">
            <button
              className="badge text-primary border-0  p-2 bg-white"
            ...
            <button className="btn btn-sm btn-danger ml-5" onClick={removeCart}>
              Remove
            </button>
          </div>
        </div>
